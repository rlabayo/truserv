    ...
    <div class="row col-lg-12">
        <div class="col-xs-12" id="create-div-1">
            <div class="panel panel-info">
                <div class="panel-heading">Korean Report</div>
                <div class="panel-body">
                    <form class="form-horizontal">
                        ...
                        <div class="form-group form-group-sm">
                            ...
                            <div class="col-sm-2">
                                <select class="form-control principal_name_select" id="principal_personal_number" name="principal_personal_number">
                                    <option value="">Select Principal Name</option>
                                    <?php
                                        foreach( $data['koreanData'] as $item ){
                                          echo "<option value=\"".$item->personal_number."\">".$item->principal_name."</option>";
                                        }
                                    ?>
                                </select>
                            </div>
                    ...
                    <div class="table-responsive col-lg-12">
                        ...
                        <table class="table table-striped table-bordered table-hover no-footer" id="example">
                            ...
                            <tr>
                                <th>#</th>
                                <th>Principal's Personal Number</th>
...
<script type="text/javascript">
    ...
    $('#view').click(function(){
        ...
        var principal_personal_number = $('#principal_personal_number').val();
        $('.download_button').remove();
        $('.tr_added').remove();
        $.ajax({
            type: "POST",
            url: "<?php echo base_url(); ?>index.php/report/getKoreanReport",
            data: {
                'startDate'                 : start_date,
                'endDate'                   : end_date,
                'dateType'                  : date_type,
                'principalPersonalNumber'   : principal_personal_number
            },
            dataType: 'json',
            success: function(data){
                $('#tbody_korean tr').remove();
                var html = '';
                if( (data.result).length > 0 ){
                    $.each(data.result, function( key, item){
                        ...
                        $('#tbody_korean').append("<tr class='tr_added'>" +
                            "<td>"+(key+1)+"</td>" +
                            "<td>"+dependent+"</td>" +
