                ...
                <div class="panel-body">
                    <form class="form-horizontal" >
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                <input type="hidden" id="tempId" value="<?php echo $data[0]->id; ?>">
                                <input type="text" class="form-control" id="firstName" placeholder="First Name" name="firstName" value="<?php echo $data[0]->firstName; ?>" required />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class = "col-sm-1 control-label" for="">Last Name</label>
                            <div class="col-sm-3">
                                <input type="text" class="form-control" id="lastName" placeholder="Last Name" name="lastName" value="<?php echo $data[0]->lastName; ?>" required />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class = "col-sm-1 control-label" for="">User Name</label>
                            <div class="col-sm-3">
                                <input type="text" disabled class="form-control" id="userName" placeholder="User Name" name="userName" value="<?php echo $data[0]->user_name; ?>" required />
                            </div>
                        </div>
                        <div class="form-group">
                            <label class = "col-sm-1 control-label" for="">Password</label>
                            <div class="col-sm-3">
                                <input type="password" class="form-control" id="password" placeholder="Password" name="password" value="<?php echo $data[0]->user_password; ?>" required />
                            </div>
                        </div>
                        <div class="form-group">
                            <div class="col-sm-4">
                                <button class="btn btn-primary btn-sm pull-right" id="update" type="button">Update Record</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<script>
    $('#update').click(function(){
        if( $('#firstName').val() == '' ){
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>update_user_settings",
                data: {
                ...
                success: function (data) {
                    if (data.status == true) {
                        ...
                        window.location.assign("<?php echo base_url(); ?>settings/" + $('#userName').val());
                    } else {
                        alert('Error found while adding new item.');
                    }
                },
